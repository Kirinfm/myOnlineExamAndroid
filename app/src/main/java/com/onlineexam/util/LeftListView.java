...
public class LeftListView extends ListView {
    ...
    public LeftListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void scrollByDistanceX() {
        // 如果向左滚动的距离大于屏幕的三分之二，就让其删除
        if (itemView.getScrollX() >= screenWidth * 2 / 3) {
            scrollLeft();
        } else if (itemView.getScrollX() <= -screenWidth * 2 / 3) {
            scrollRight();
        } else {
            // 滚回到原始位置
            scrollBack();
